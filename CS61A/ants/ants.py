...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            ...
            if current_bees and self.lower_bound <= distance <= self.upper_bound:
                return random_bee(current_bees)
    ...
    def reduce_health(self, amount):
        ...
        if bees:
            if self.health - amount <= 0:
                attack_amount = self.damage + amount
            else:
                attack_amount = amount
            for bee in bees[:]:
                bee.reduce_health(attack_amount)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while pointer:
            if pointer.ant:
                pointer.ant.double()
                if pointer.ant.is_container and pointer.ant.ant_contained:
                    pointer.ant.ant_contained.double()
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            ...
            def slow_action(gamestate):
                if target.slow_state > 0:
                    if target.slow_state % 2 == 0:
                        origin(target, gamestate)
                    target.slow_state -= 1
                else:
                    origin(target, gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        distance = 0
        pointer = self.place
        while not pointer.is_hive:
            ant = pointer.ant
            if ant:
                if distance > 0:
                    insect_dict[ant] = distance
                    if ant.is_container:
                        insect_dict[ant.ant_contained] = distance
                else:
                    if pointer.ant is not self:
                        insect_dict[self.place.ant] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(self.damage - distance * 0.25 - 0.0625 * self.insects_shot, 0)
        # END Problem Optional 2
