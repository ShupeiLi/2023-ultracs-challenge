...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        t = insert_into_all(s[0], subseqs(s[1:]))
        return t + subseqs(s[1:])
#       t = [[]]
#       for i in range(len(s) - 1, -1, -1):
#           t.extend(insert_into_all(s[i], t))
#       return t
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            return [[]]
        elif s[0] <= prev: # Bug: < to <=
            return insert_into_all(s[0], subseq_helper(s[1:], s[0])) + subseq_helper(s[1:], s[0])
        else:
            a = s[0]
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    """
    # "*** YOUR CODE HERE ***"
    # Bug: Without the use of recursion
    def factorial(n):
        total = 1
        k = 2
        while k <= n:
    ...
    if n <= 2:
        ...
        m = n - 1
        return int(factorial(2 * m) / (factorial(m) * factorial(m + 1)))
...
class VendingMachine:
    ...
    """
    # "*** YOUR CODE HERE ***"
    restock_hint = 'Nothing left to vend. Please restock.'
    ...
    def add_funds(self, money):
        if self.stock == 0:
            return self.restock_hint + f' Here is your ${money}.'
        else:
            self.balance += money
...
def long_paths(t, n):
    ...
    """
    # "*** YOUR CODE HERE ***"
    # Reference: https://github.com/MartinLwx/CS61A-Fall-2021-UCB/blob/main/Labs/lab09.py
    paths = list()
    def helper(t, current_path, n):
        nonlocal paths
